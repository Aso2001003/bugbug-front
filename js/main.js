...
window.addEventListener("load", () => {
  ...
  const recipeImgArray = document.querySelectorAll(".popRecipe__list__item__img");
  ...
  insectFilter.addEventListener("click", () => {
    ...
    for (let i = 0, size = recipeImgArray.length; i < size; i++) {
      if (filterStarte) {
        recipeImgArray[i].classList.add("js-filter");
      } else {
        recipeImgArray[i].classList.remove("js-filter");
      }
    }
  });
